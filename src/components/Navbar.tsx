...
function NavListMenu({ scrolled }: { scrolled: boolean }) {
  ...
  return (
    ...
    >
      <div
        className={`font-bold inline-flex items-center gap-2 cursor-pointer text-center ${
          scrolled ? "text-gray-400 hover:text-primary-700" : ""
        }`}
      >
        Programas
        <svg
      ...
      >
        <div className="flex items-center justify-start flex-col p-4 text-white">
          {programs.map((item, index) => (
            <a
              className="flex justify-start items-center w-full py-2 px-4 align-middle hover:bg-gray-100 hover:text-primary-500 rounded-md"
              href={item.path}
              key={index}
            >
...
export default function Navbar({ logoLight, logoDark, routePath }: Props) {
  ...
  return (
    ...
    >
      <nav
        className={`p-4 max-w-7xl xl:max-w-full mx-auto ${
          openNav ? "h-screen" : "h-auto"
        }`}
      >
        <div className="flex items-center justify-between lg:justify-around select-none">
          <a href="/" className="text-primary-500">
            <img
              loading="eager"
              src={logoDark}
              className={`h-16 ${scrolled ? "hidden" : ""}`}
              alt="Logo Amazon"
            />
            <img
              loading="lazy"
        ...
        >
          <div
            className={`flex gap-8 items-center justify-center flex-col py-8 h-full`}
          >
